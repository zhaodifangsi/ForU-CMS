...
if (isset($_POST['submit'])) {
  ...
  $data['a_func'] = $_POST['a_func'];
  $data['a_desc'] = $_POST['a_desc'];
  $data['a_order'] = $_POST['a_order'];
  $data['a_enable'] = $_POST['a_enable'];
  $data['a_safe'] = $_POST['a_safe'];
          ...
          <main class="am-panel-bd am-collapse am-in am-scrollable-horizontal" id="collapse-panel-1">
            <table class="am-table am-table-striped am-table-bordered am-table-compact admin-content-table am-text-nowrap">
              ...
              <tbody>
                <?php
                $page = new Page(20);
                $page->handle($db->getOne("SELECT COUNT(*) FROM addon"));
                $res = $db->getAll("SELECT * FROM addon ORDER BY a_order ASC,id DESC LIMIT ".$page->page_start.",".$page->page_size);
                if (!empty($res)) {
                  foreach ($res as $row) {
