          ...
          <main class="am-panel-bd am-collapse am-in am-scrollable-horizontal" id="collapse-panel-1">
            ...
            <table class="am-table am-table-striped am-table-bordered am-table-compact admin-content-table am-text-nowrap">
              ...
              <tbody>
                ...
                $page = new Page(20);
                $page->handle($db->getOne("SELECT COUNT(*) FROM detail $where"));
                $res = $db->getAll("SELECT * FROM detail $where ORDER BY id DESC LIMIT ".$page->page_start.",".$page->page_size);
                if (!empty($res)) {
                foreach($res as $row){
                ?>
                <tr>
            ...
            <div data-am-page="{pages:<?php echo $page->page_sum;?>,curr:<?php echo $page->page_current;?>,jump:'?cid=<?php echo str_safe($cid);?>&page=%page%'}"></div>
          </main>
        </section>
