...
if (isset($_POST['submit'])) {
  ...
  $a_npassword = $db->getOne("SELECT u_psw FROM user WHERE id = " . $_GET['id']);
  if ($a_password == '') {
  ...
  $sql = "UPDATE user SET u_rid=" . $a_role . ",u_name='" . $a_name . "',u_tname='" . $a_tname . "',u_psw='" . $password . "' WHERE id = " . $_GET['id'];
  ...
  <div class="admin-content">
    <div class="am-g am-g-fixed">
      <div class="am-u-sm-12 am-padding-top">
        <?php
        $res = $db->
        getRow("SELECT * FROM user WHERE id = " . $_GET['id']);
        if ($row = $res) {
        ?>
