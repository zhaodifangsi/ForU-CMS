...
foreach ($res as $val) {
  $str .= '  <url>'.PHP_EOL.'    <loc>' . $cms['s_domain'] . '/' . d_url($val['id']) . '</loc>'.PHP_EOL.'    <lastmod>' . local_date("Y-m-d H:i:s",$val['d_date']) . '</lastmod>'.PHP_EOL.'    <changefreq>' . $changefreq . '</changefreq>'.PHP_EOL.'    <priority>0.8</priority>'.PHP_EOL.'  </url>'.PHP_EOL;
}
$str .= '</urlset>';
