...
class Channel {
  ...
  public function getAllMain($nav=1, $order="c_order ASC,id ASC") {
    if ($nav) {
      $res = $this->db->getAll("SELECT * FROM channel WHERE c_main = 0 AND c_navigation = 1 ORDER BY $order");
    } else {
      $res = $this->db->getAll("SELECT * FROM channel WHERE c_main = 0 ORDER BY $order");
    }
    return $res;
  }
  // 获取所有频道
