...
class User {
  ...
  public function login($t=''){
    ...
    $u_psw = isset($_POST['u_psw']) ? md5($_POST['u_psw']) : '';
    if (check($u_email, 'email')) {
      $res = $this->db->getRow("SELECT * FROM user WHERE u_email = '" . $u_email . "' AND u_psw = '" . $u_psw . "'");
    } else {
      $res = $this->db->getRow("SELECT * FROM user WHERE u_mobile = '" . $u_email . "' AND u_psw = '" . $u_psw . "'");
    }
  ...
  public function addAddr($arr) {
    ...
    return $this->db->autoExecute('user_address', $arr, 'INSERT');
  ...
  public function addPoint($point) {
    ...
    $user_point = intval($point) + intval($user['u_point']);
    //计算总积分
  ...
  // 扣除积分函数
  public function costPoint($point) {
    non_numeric_back($point, $GLOBALS['lang']['uc']['point']['error']);
    //判断是否数字
    if ($point > $user['u_point']) {
      url_back($GLOBALS['lang']['uc']['point']['cost_faild']);
    } else {
      $user_point = intval($user['u_point']) - intval($point);
      //计算总积分
      if ($this->db->query("UPDATE user SET u_point = '$user_point' WHERE id = " . $this->user_id)) {
        url_back();
      }
