  ...
  foreach ($res as $val) {
    if ($val['c_ifsub'] || $val['c_parent']==0) {
      if ($val['c_ifsub']==0 && $val['c_parent']==0) {
        $str .= '<table class="am-table am-table-bordered"><tr><th><label for="c'.$val['id'].'"><input type="checkbox" name="c'.$val['id'].'" id="c'.$val['id'].'" value="c'.$val['id'].'" '.(in_array("c".$val['id'],$priv) ? 'checked="checked"' : '').'>'.$val['c_name'].'</label></th><td class="gray">无子频道</td></tr></table>';
      } else {
        $str .= '<table class="am-table am-table-bordered"><tr><th><label for="c'.$val['id'].'"><input type="checkbox" name="c'.$val['id'].'" id="c'.$val['id'].'" value="c'.$val['id'].'" '.(in_array("c".$val['id'],$priv) ? 'checked="checked"' : '').'>'.$val['c_name'].'</label></th><td>'.getChannelPriv($val['id'],$rid,$priv).'</td></tr></table>';
      }
    } else {
