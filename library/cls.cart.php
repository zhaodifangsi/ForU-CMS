...
class Cart {
  ...
  private function has_options($rowid = '') {
    if( ! isset($this->_cart[$rowid]['options']) OR count($this->_cart[$rowid]['options']) === 0) {
      return FALSE;
    }
  ...
  private function _update($items = array()) {
    //输入物品参数异常
    if( ! isset($items['rowid']) OR ! isset($items['qty']) OR ! isset($this->_cart[$items['rowid']])) {
      if($this->debug == TRUE) {
        $this->_log("cart_items_data_invalid");
      }
      return FALSE;
  ...
  private function _save_cart() {
    ...
    if(comCartDB()) $this->cartDB($this->_cart);
    return TRUE;
  }
