...
function limitUserLogin($u_id) {
  ...
  if($session_id = get_col("user_session", "session_id", "u_id = ".$u_id)) {
    // var_dump($arr['session_id'], $session_id);
...
function getCartInfo($cart) {
  ...
  foreach ($cart as $val) {
    foreach ($val as $k => $v) {
      // $k = array(rowid,id,qty,price,name,options,subtotal);
      if(in_array($k, $exclude)!==false) continue;
      $str.= $v.',';
    }
    $str = rtrm($str,",").'|';
  }
...
function getCartItem($t=1) {
  ...
  foreach ($arr as $key => $value) {
    ...
    if($t) $res[$key]['subtotal'] = $arr_info[4];
  }
  return $res;
}
